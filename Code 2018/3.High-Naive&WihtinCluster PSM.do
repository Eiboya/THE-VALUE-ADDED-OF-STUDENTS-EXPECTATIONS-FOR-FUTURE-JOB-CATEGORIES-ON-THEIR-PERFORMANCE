...
/* ==PART II. Naive Propensity Score Methods - High==
...
/* ==PART III. Within-Cluster Propensity Score Methods - High==
...
*** ==PART II. Naive Propensity Score - High==
...
gen attweight = .
replace attweight =(High ==1) if High ==1
replace attweight =(High ==0)*pscore1/(1- pscore1) if High ==0


forvalues i=1(1)10 {
  reg PV`i'SCIE [pw=attweight] if High==1, cluster(CNTSCHID)
  gen treated`i'=_b[_cons]
  reg PV`i'SCIE [pw=attweight] if High==0, cluster(CNTSCHID)
  gen controls`i'=_b[_cons]
  reg science High [pw=attweight], cluster(CNTSCHID)
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High  [fweight = _weight] if _weight!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High [fweight = _weight] if _weight!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High $x [fweight = _weight] if _weight!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High $x [fweight = _weight] if _weight!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
*psmatch2 High, pscore(pscore1) outcome(PV1SCIE) ate common neighbor(1) noreplacement caliper(`=scalar(cal)') 

forvalues i=1(1)10 {
  psmatch2 High, pscore(pscore1) outcome(PV`i'SCIE) common neighbor(1)  noreplacement caliper(`=scalar(cal)') 
  gen ATE`i'=r(ate)
  gen ATT`i'=r(att)
  gen SDATT`i'=r(seatt)
  sum PV`i'SCIE if _treated==1 & _support==1
...
forvalues i=1(1)10 {
  psmatch2 High, pscore(pscore1) outcome(PV`i'SCIE) common neighbor(2) caliper(`=scalar(cal)') 
  gen ATE`i'=r(ate)
  gen ATT`i'=r(att)
  gen SDATT`i'=r(seatt)
  sum PV`i'SCIE if _treated==1 & _support==1
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High  [fweight = _weight2] if _weight!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High  [fweight = _weight2] if _weight!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High $x [fweight = _weight2] if _weight!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High $x [fweight = _weight2] if _weight!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
*** ==PART III. Within-Cluster Propensity Score - High==
clear
use "C:\Users\dhxht\Desktop\Zhihan Zhang - Master 2 Thesis\WORLDstudent2018.dta"
...
foreach j of local cluster {
  ...
  sum pscore`j' if c==`j'
  replace attweight =(High ==1) if High ==1 & c==`j'
  replace attweight =(High ==0)*(pscore`j')/(1- pscore`j') if High ==0 & c==`j'
  }
...
forvalues i=1(1)10 {
  foreach j of local cluster {
  reg PV`i'SCIE [pw=attweight] if High==1 & (c==`j'), cluster(CNTSCHID) 
  replace treatedoutcome`i'=_b[_cons] if (c==`j') 
  reg PV`i'SCIE [pw=attweight] if High==0 & (c==`j'), cluster(CNTSCHID)
  replace controlsoutcome`i'=_b[_cons] if (c==`j') 
  }
  reghdfe science High [pw=attweight], cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  }
...
sum support1 if High==1 & support1==1
sum support1 if High==0 & support1==1
sum support1 if support1==0
sum support1 if High==1 & support1==0
sum support1 if High==0 & support1==0
sum weight1
sum weight1 if High==1
sum weight1 if High==0
total weight1

forvalues i=1(1)10 {
  reg PV`i'SCIE High  [fweight = weight1] if weight1!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High  [fweight = weight1] if weight1!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High $x [fweight = weight1] if weight1!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High $x [fweight = weight1] if weight1!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
qui foreach j of local cluster {	
	psmatch2 High if  (c==`j'), pscore(pscore`j') outcome(PV`i'SCIE) common neighbor(1) noreplacement caliper(`=scalar(kkcal_`j')') 
	replace pATE`i'=r(ate) if (c==`j')
	replace pATT`i'=r(att) if  (c==`j')
	sum PV`i'SCIE if _treated==1 & _support==1 &  (c==`j')
	replace ptreated`i'=r(mean) if  (c==`j')
...
forvalues i=1(1)10 {
qui foreach j of local cluster {	
	psmatch2 High if  (c==`j'), pscore(pscore`j') outcome(PV`i'SCIE) common neighbor(2) caliper(`=scalar(kkcal_`j')') 
	replace pATE`i'=r(ate) if (c==`j')
	replace pATT`i'=r(att) if  (c==`j')
	sum PV`i'SCIE if _treated==1 & _support==1 &  (c==`j')
	replace ptreated`i'=r(mean) if  (c==`j')
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High  [fweight = weight12] if weight1!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High  [fweight = weight12] if weight1!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reg PV`i'SCIE High $x [fweight = weight12] if weight1!=., cluster(CNTSCHID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
...
forvalues i=1(1)10 {
  reghdfe PV`i'SCIE High $x [fweight = weight12] if weight1!=., cluster(CNTSCHID) absorb(CNTRYID)
  gen treatment`i'=_b[High]
  gen SDtreatment`i'=_se[High]
  gen cons`i'=_b[_cons]
  gen SDcons`i'=_se[_cons]
